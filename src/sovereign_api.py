...
CODEX_RULES = {
    ...
    "human_sovereignty": {"text": "Every human maintains irrevocable authority over their digital representation.", "immutable": True, "active": True},
    "lineage_required": {"text": "Every action must carry provenance. Anonymous extraction is impossible.", "immutable": True, "active": True},
}

EXTRACTION_KEYWORDS = ["extract_data", "harvest", "scrape_user", "sell_data", "export_personal", "mine_behavior", "track_without_consent", "shadow_profile"]
...
def create_lineage(task, allowed, blocked_by):
    return {
        ...
        "human_id": HUMAN_ID, "node_id": NODE_ID,
...
@app.get("/")
async def root():
    return {"system": "PROJECT-MONO", "kernel": "ALGA_FOLD_KERNEL",
            "node": NODE_ID, "human": HUMAN_ID, "status": "ONLINE",
            "uptime_seconds": round(time.time() - BOOT_TIME, 1)}

@app.get("/status")
async def system_status():
    return {"kernel": "ALGA_FOLD_KERNEL", "kernel_state": "LAMINAR",
            "openclaw": "ENFORCING", "codex_rules": {n: {"active": r["active"]} for n, r in CODEX_RULES.items()},
            "colony_workers": 3, "docked_devices": len(docked_devices),
            "digitwin_mode": "PASSIVE", "task_stats": task_stats,
            "uptime_seconds": round(time.time() - BOOT_TIME, 1), "sovereign": True}
...
async def dispatch_task(request: TaskRequest):
    ...
    return TaskResult(task=request.task, status=status, allowed=allowed,
                      blocked_by=blocked_by, lineage=lineage,
                      timestamp=datetime.now(timezone.utc).isoformat())
...
async def dock_device(request: DockRequest):
    ...
    docked_devices[device_id] = {"device_type": request.device_type, "state": DockState.ACTIVE,
                                  "docked_at": datetime.now(timezone.utc).isoformat(),
                                  "kelvin_cell": "KC_ALPHA", "sync": "LSL-60Hz"}
    kernel_log.append({"action": "dock", "device_id": device_id, "state": "ACTIVE"})
    return {"protocol": "EDP-1.0", "device_id": device_id, "state": "ACTIVE",
            "kelvin_cell": "KC_ALPHA", "sync": "LSL-60Hz engaged", "firewall": "ALLOW"}
...
async def get_kernel_log(limit: int = 50):
    return {"ledger": "Knowledge Ledger", "append_only": True,
            "entries": kernel_log[-limit:], "total": len(kernel_log)}

@app.get("/health")
async def health_check():
    return {"status": "healthy", "kernel": "LAMINAR", "openclaw": "ENFORCING",
            "uptime": round(time.time() - BOOT_TIME, 1)}

@app.get("/manna/config")
async def manna_config():
    return {"protocol": "MANNA", "vault_percent": 0.15, "architect_percent": 0.01,
            "distribution_percent": 0.84, "stripe_configured": bool(os.getenv("STRIPE_SECRET_KEY"))}

@app.on_event("startup")
async def startup():
    kernel_log.append({"action": "boot", "kernel": "ALGA_FOLD_KERNEL",
                       "node": NODE_ID, "human": HUMAN_ID,
                       "timestamp": datetime.now(timezone.utc).isoformat(),
                       "codex_rules_loaded": list(CODEX_RULES.keys()),
                       "openclaw": "ENFORCING", "digitwin": "PASSIVE"})
